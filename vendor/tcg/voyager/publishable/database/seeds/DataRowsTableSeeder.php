...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $discussionDataType = DataType::where('slug', 'discussions')->firstOrFail();
        $categoryDataType = DataType::where('slug', 'categories')->firstOrFail();
        $userDataType = DataType::where('slug', 'users')->firstOrFail();
        $menuDataType = DataType::where('slug', 'menus')->firstOrFail();
        $roleDataType = DataType::where('slug', 'roles')->firstOrFail();

        $dataRow = $this->dataRow($discussionDataType, 'id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'number',
                'display_name' => 'ID',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'user_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'Author',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'category_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'sub_category_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'title');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'Title',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'content');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'rich_text_box',
                'display_name' => 'Body',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'slug');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'slug',
                ...
                'details'      => '
{
    "slugify": {
        ...
        $dataRow = $this->dataRow($discussionDataType, 'status');
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'details'      => '
{
    ...
    "options": {
        ...
        $dataRow = $this->dataRow($discussionDataType, 'created_at');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
        ...
        $dataRow = $this->dataRow($discussionDataType, 'updated_at');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'add'          => 1,
                'delete'       => 1,
                'details'      => '/storage/avatars/default-M.png',
                'order'        => 11,
            ])->save();
        }
    ...
    $dataRow = $this->dataRow($categoryDataType, 'num_of_discussions');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'number',
                'display_name' => 'Num of discusions',
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($categoryDataType, 'created_by_user_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'number',
                'display_name' => 'created_by_user_id',
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
